...
tree<-read.tree("./data/Abt_NJ_tree_rpoB.nwk") 
outgroup.name<-"EU477149.2AcinetobactercalcoaceticusstrainNIPH2245RNApolymerasesubunitB(rpoB)genepartialcds"
...
library(readxl) 
library(stringr)
accession.numbers <- read_excel(".data/Abt_ANs_data.xlsx", sheet = "Abt_metadata")
...
traits <- read_excel("./data/Abt_ANs_final_data.xlsx", sheet = "Z-scores")
...
df_PICs <- data.frame(matrix(unlist(PICs), ncol=ncol(traits), byrow=F)) 
colnames(df_PICs)<-colnames(df_PICs)
...
library(xlsx)
write.xlsx(r, file="./results/Abt_PICs_correlation.xlsx", sheetName="Spearman corr coef", row.names=TRUE)
write.xlsx(P, file="./results/Abt_PICs_correlation.xlsx", sheetName="p-values", append=TRUE, row.names=TRUE)
write.xlsx(Padj, file="./results/Abt_PICs_correlation.xlsx", sheetName="adjusted p-values", append=TRUE, row.names=TRUE)
